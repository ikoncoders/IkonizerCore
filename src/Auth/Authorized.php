...
namespace IkonizerCore\Auth;

use IkonizerCore\Base\Exception\BaseUnexpectedValueException;
use IkonizerCore\Auth\Model\RememberedLoginModel as RememberedLogin;
use IkonizerCore\Base\Exception\BaseNoClassFoundExeption;
use IkonizerCore\Session\SessionTrait;
use IkonizerCore\Cookie\CookieFacade;
...
{ 
    ...
    private static function userModel(): UserModel
    {
        if (!class_exists($userModel = UserModel::class)) {
            throw new BaseNoClassFoundExeption(sprintf('%s class is missing. In order to utalize the Authorized script within IkonizerCore. Then a user model is required.', UserModel::class));
        }
